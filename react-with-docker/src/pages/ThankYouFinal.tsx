...
const ThankYouPage = () => {
  ...
  const eventDateRange = formData?.eventDateRange || [];
  const formattedDates = eventDateRange.map(dateObj => {
    const date = new Date(dateObj.$d);
    return date.toLocaleDateString('en-US', {
      weekday: 'long', // Display weekday
      year: 'numeric', // Display year
      month: 'long', // Display month in long format
  ...
  return (
    <Box sx={{ 
      ...
      <Paper sx={{ maxWidth: 900, width: '100%', borderRadius: 3, boxShadow: 5, padding: 4 }}>
          ...
          <CardContent>
            ...
            {/* <Grid container spacing={2} sx={{ marginY: 2 }}>
              ...
              <Grid item xs={12} sm={6}>
                ...
                <Grid container spacing={0} sx={{ marginY: 2 }}>
    <Grid item xs={4}>
        <Typography variant="body1"><strong>Location:</strong> {formData?.location}</Typography>
    </Grid>
    <Grid item xs={4}>
    ...
    <Grid item xs={4}>
        <Typography variant="body1"><strong>Additional Info:</strong> {formData?.additionalInfo}</Typography>
            ...
            {/* Displaying the formatted event dates */}
              ...
              <>
                ...
                <List sx={{ backgroundColor: 'rgba(0, 0, 0, 0.05)', borderRadius: 2 }}>
                  {formattedDates.map((date:any, index:any) => (
                    <ListItem key={index} sx={{ marginBottom: 1, borderRadius: 1 }}>
                      <ListItemText primary={date} sx={{ fontWeight: 'bold' }} />
                    </ListItem>
                  ))}
                </List>
              </>
